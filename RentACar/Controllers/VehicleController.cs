...
namespace RentACar.Controllers
{
	...
	public class VehicleController : Controller
	{
		...
		[HttpGet]
		public async Task<IActionResult> VehiclesForService(string branchId)
		{
		...
		[HttpPost]
		public async Task<IActionResult> ServiceVehicle(string branchId, string vehicleId)
		{
			(bool isSuccessful, ServiceVehicleViewModel servicedVehicle) = await vehicleService.ServiceVehicleAsync(branchId, vehicleId);
			if (isSuccessful == false)
			{
				return NotFound(); //Ids are not correct
			}
			if (!isSuccessful)
			{
				...
				return RedirectToAction("VehiclesForService", "Vehicle", branchId);
			}

			TempData["Message"] = $"You have successfully serviced {servicedVehicle.Make} {servicedVehicle.Model}.";
			TempData["MessageType"] = "Success";

			return RedirectToAction("VehiclesForService", "Vehicle", new { branchId });
			return this.View();
		}

		public async Task<IActionResult> VehiclesRentalEndingToday()
		{
			return this.View(); //TODO:Implement
		}
